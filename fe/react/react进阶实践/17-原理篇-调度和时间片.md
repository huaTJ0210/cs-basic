...
> - 时间切片：划分时间段，避免某些任务长期占着主线程导致其他高优先级的任务无法得到立即执行，这种方式能够避免长时间运行任务堵塞主线程，从而提高应用的整体响应和性能；
> - 基本调度流程：
>   - 创建的新任务（优先级+callback）,经过 Scheduler 的包装为一个 Task（id，callback，experiationTime，startTime），
>   - 区分立即执行的任务和延期执行的任务：立即执行的 Task 放入到 TaskQueue（最小堆实现的具有优先级的队列）中，然后进行调度；
>   - 任务的执行要在一个宏任务队列中（采用的是 messageChannel 实现的宏任务队列）
>   - 各个任务完成后，都要检查是否需要将控制权转交给主线程
> - 合作式调度和抢占式调度
>   - 抢占式造成任务饿死， scheduler 会标记这些任务并提高优先级

在 React 18 中，`Scheduler` 是一个关键模块，它负责调度任务的执行，实现了时间切片和优先级调度等功能，使得 React 能够更高效地处理渲染任务，避免长时间阻塞主线程。下面将深入讲解 `Scheduler` 的源码实现。



### 整体架构

`Scheduler` 主要包含以下几个核心部分：

1. **任务队列管理**：负责存储不同优先级的任务。
2. **时间管理**：记录任务的开始时间、过期时间等。
3. **调度循环**：不断从任务队列中取出任务并执行。
4. **优先级管理**：为不同类型的任务分配不同的优先级。

### 关键数据结构

#### 1. `Task` 对象

每个任务都是一个 `Task` 对象，包含以下属性：

```javascript
// 任务对象
function Task(priorityLevel, callback, startTime, expirationTime) {
  this.priorityLevel = priorityLevel; // 任务优先级
  this.callback = callback; // 任务回调函数
  this.startTime = startTime; // 任务开始时间
  this.expirationTime = expirationTime; // 任务过期时间
  this.sortIndex = -1; // 用于排序的索引
}
```
...
- `timerQueue`：存储尚未到开始时间的任务。
- `taskQueue`：存储已经到开始时间的任务。
...
不同优先级的任务具有不同的过期时间，过期时间越短，优先级越高。

### 调度循环

#### 1. `requestHostCallback` 函数

该函数用于请求执行一个回调函数，会将任务添加到任务队列中，并启动调度循环。

```javascript
function requestHostCallback(callback) {
  const currentTime = getCurrentTime();
  const startTime = currentTime;
  const expirationTime = startTime + TIMEOUTS[NormalPriority]; // 普通优先级任务的过期时间
  const newTask = new Task(NormalPriority, callback, startTime, expirationTime);
  // 将任务添加到任务队列
...
#### 2. `schedulePerformWorkUntilDeadline` 函数

该函数会根据当前环境选择合适的调度方式，在浏览器环境中使用 `MessageChannel` 来实现异步调度。
...
#### 3. `performWorkUntilDeadline` 函数

该函数是调度循环的核心，会不断从任务队列中取出任务并执行，直到没有任务或者达到时间切片的限制。

```javascript
function performWorkUntilDeadline() {
  // 获取当前时间
  const currentTime = getCurrentTime();
  // 将 timerQueue 中已经到开始时间的任务移动到 taskQueue 中
  advanceTimers(currentTime);
  // 检查 taskQueue 中是否有任务
  if (taskQueue.length > 0) {
    // 取出优先级最高的任务
    const highestPriorityTask = taskQueue[0];
    const expirationTime = highestPriorityTask.expirationTime;
    let hasTimeRemaining = true;
    let currentTask = highestPriorityTask;
    do {
      ...
      (hasTimeRemaining || currentTime >= expirationTime) &&
      taskQueue.length > 0
    );
  }
  // 检查是否还有任务需要执行
  if (taskQueue.length > 0 || timerQueue.length > 0) {
    // 继续调度
    schedulePerformWorkUntilDeadline();
  }
}
...
`Scheduler` 使用 `performance.now()` 来获取当前时间，用于计算任务的开始时间和过期时间。
...
`Scheduler` 的核心思想是将任务按照优先级和时间进行管理，通过时间切片的方式，在不阻塞主线程的情况下，高效地执行任务。主要步骤包括：

1. 将任务添加到任务队列中。
2. 启动调度循环，使用 `MessageChannel` 实现异步调度。
3. 在调度循环中，不断从任务队列中取出任务并执行，直到没有任务或者达到时间切片的限制。
4. 检查是否还有任务需要执行，如果有则继续调度。

通过这种方式，React 能够在复杂的应用场景下，保持界面的流畅性和响应性
